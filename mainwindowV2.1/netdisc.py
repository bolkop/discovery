...
class netdisc:
    ...
    def do_lcfg(self, device, configFile):
        ...
        Raise exception if not able to execute command
        """
     #   print ("configFile in lcfg", configFile)
    ...
    def do_rcfg(self, device, macAddress, configFile):
        ...
        Raise exception if not able to execute command
        """
       # print ("configFile in rcfg", configFile)
        try:
            configs = open(configFile, "rb")
        except:
            raise ValueError("Cannot open the config file")
    ...
    def do_print(self, dev=None):
        ...
        tmpSummary = "Device Summary - DEV-7 192.168.0.150 00:aa:de:00:00:52 \n" \
                     "Interfaces: Name IP Address MAC Address MTU Netmask    Broadcast\n" \
    ...
    def reset(self, dev, network):
        """
        Modem software reset
        """
        
        print("reset device ", dev, network)
        # Response from netdisc
        readback = self.__do_action("reset", dev, network)
    ...
    def setIp(self, mac, ip=None, mask=None, gateway=None):
        errorCode = self.__pcControl.setIp(mac, ip, mask, gateway)
        if errorCode == 0:
            # Extra time to make sure that IP is properly set
            time.sleep(1)
    ...
    def set_pcXML(self, mac, ip=None, mask=None):
        ...
        # MAC address not change and return False
         ...
         #   print("{e} {f} is malformed or not an xml".format(e=err, f=tree))
            return False
